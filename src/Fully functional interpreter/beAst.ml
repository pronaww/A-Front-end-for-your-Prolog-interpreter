...
type term = Var of string | Const of exp | Func of string * (term list) | List of term list | HdTl of term * (term list)
...
let rec mgu' t u (l, boo) = let boolXinL = checkPresent' t l and boolYinL = checkPresent' u l in
	match (t,u) with
	(Var(x),Var(y)) -> if x = y then (l, boo) else if boolXinL=false && boolYinL=false then (((Var(x),Var(y))::l), boo)
...
| (Const(a), Const(b)) -> if a = b then (l,boo) else (l, false)
| (Func(str, termList), Const(a))|(Const(a), Func(str, termList)) -> (l, false)
| (Func(str,termList1),Func(g,termList2)) -> if str=g && (length termList1)=(length termList2) then (fy mgu' termList1 termList2 (l,boo)) else (l, false);;
